 ...
 */
export async function getDeathTileIndex(
  serverSeed: string, 
  rowIndex: number, 
  tiles: number
): Promise<number> {
 ...
 */
export async function generateRowCount(serverSeed: string): Promise<number> {
 ...
 */
export async function calculateAllDeathTiles(
  serverSeed: string,
  boardLayout: number[]
): Promise<Record<number, number>> {
  const deathTiles: Record<number, number> = {};
 ...
 */
export async function verifyServerCommit(
  serverSeed: string, 
  serverCommit: string
): Promise<boolean> {
