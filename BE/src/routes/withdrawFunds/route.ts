...
import { Connection, PublicKey, Keypair, SystemProgram, Transaction, LAMPORTS_PER_SOL, sendAndConfirmTransaction } from "@solana/web3.js"; // Added sendAndConfirmTransaction
...
// import { MethodsNamespace } from "@coral-xyz/anchor"; // Attempting import from top-level anchor package
...
const provider: anchor.AnchorProvider = new anchor.AnchorProvider(connection, wallet, anchor.AnchorProvider.defaultOptions()); // Uncommented and explicitly typed
    ...
    try {
        ...
        try {
            ...
            await connection.confirmTransaction(
                { signature: withdrawTx, lastValidBlockHeight: backendLastValidBlockHeight, blockhash: blockhash },
                'confirmed'
            );
        ...
        if(updatedUser == null) return res.status(400).json({ success: false, message: "User not found after update" });
        ...
        if (error.logs && Array.isArray(error.logs)) {
            const solanaError = error.logs.find((log: string) => log.includes("Program failed"));
            if (solanaError) {
