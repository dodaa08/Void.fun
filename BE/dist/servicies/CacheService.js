...
export const setCacheBatch = async (operations) => {
    try {
...
export const getCacheBatch = async (keys) => {
    try {
        const pipeline = redisClient.multi();
        keys.forEach(key => pipeline.get(key));
        const results = await pipeline.exec();
        return results?.map((result) => result && result[1] ? JSON.parse(result[1]) : null) || [];
    }
    catch (error) {
...
export const deleteCacheBatch = async (keys) => {
    try {
        const pipeline = redisClient.multi();
